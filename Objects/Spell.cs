...
namespace Talos.Objects
{
    internal class Spell
    {
        ...
        internal bool CanUse
        {
            get
            {
                //Console.WriteLine($"[SPELL] Checking CanUse for spell {Name}. Cooldown until: {Cooldown.AddSeconds(Ticks)}, Current Time: {DateTime.UtcNow}");

                // Check if the cooldown has expired
                if (DateTime.UtcNow >= Cooldown.AddSeconds(Ticks))
                {
                    //Console.WriteLine("[SPELL] Spell can be used. Cooldown period has passed.");
                    return true;
                }
                else
                {
                    //Console.WriteLine("[SPELL] Spell cannot be used yet. Still within cooldown period.");
                    return false;
                }
            }
        }
