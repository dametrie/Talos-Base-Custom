...
namespace Talos.Cryptography
{
    ...
    {
        private static readonly ushort[] _dialogCrcTable = new ushort[256] {
        ...
        internal static ushort Calculate(byte[] buffer) => Calculate(buffer, 0, buffer.Length);

        internal static ushort Calculate(byte[] buffer, int offset, int length)
        {
            ushort header = 0;
            for (int i = 0; i < length; i++)
                header = (ushort)(buffer[offset + i] ^ (((ushort)(header << 8)) ^ _dialogCrcTable[header >> 8]));
            return header;
        }
    }
}
